import type { AppSettings, AppSnapshot, AppTab, TunnelServer, MacApplication } from "../shared/contracts";
...
let processList: MacApplication[] = [];
  ...
  return `
    ...
    <div class="profile-grid">
      ...
      <section class="section card"><h2 class="card-title">Настройки</h2><div class="settings-list">
        ...
        ${switchSetting("Защита от DNS-утечек", "Блокировать DNS-запросы вне защищённого туннеля", "preventDnsLeaks", state.settings.preventDnsLeaks)}
        ...
        ${selectSetting("Раздельное туннелирование", "Маршрутизация по приложениям macOS", "split-tunnel-mode", state.settings.splitTunnelMode, [["off","Выключено"],["bypass","Исключить выбранные"],["only","Только выбранные"]])}
...
function renderProcessDialog(): string {
  ...
  return `<div class="dialog-backdrop" id="process-dialog-backdrop">
    <section class="process-dialog card" role="dialog" aria-modal="true" aria-labelledby="process-dialog-title">
      <header class="dialog-header"><div><h2 id="process-dialog-title">Приложения macOS</h2><p>Выберите процессы для правила раздельного туннелирования.</p></div><button class="button icon-button" id="close-process-dialog" aria-label="Закрыть">${icon("close")}</button></header>
      ...
      ${processDialogLoading ? `<div class="dialog-loading"><span class="spinner"></span> Получаем список процессов…</div>` : processList.length ? `<div class="process-list">${processList.map((item) => `<button class="process-row ${selected.has(item.path ?? item.name) ? "selected" : ""}" data-process-name="${escapeAttribute(item.path ?? item.name)}" data-process-filter="${escapeAttribute(`${item.name} ${item.path ?? ""}`.toLocaleLowerCase("ru"))}" aria-pressed="${selected.has(item.path ?? item.name)}"><span class="process-icon">${icon("process")}</span><span class="process-copy"><span class="process-name">${escapeHtml(item.name)}</span><span class="process-path">${escapeHtml(item.path ?? "Процесс сейчас не запущен")}</span></span><span class="process-check">${selected.has(item.path ?? item.name) ? icon("check") : ""}</span></button>`).join("")}</div>` : `<div class="empty">Запущенные процессы не найдены</div>`}
...
function bindPageEvents(): void {
  ...
  document.getElementById("browse-executable-button")?.addEventListener("click", () => void run(async () => {
    ...
    if (!selected) return;
    pendingProcesses.add(selected.path ?? selected.name);
...
async function openProcessDialog(): Promise<void> {
  ...
  try {
    ...
    const runningNames = new Set(running.map((item) => item.path ?? item.name));
    processList = [
      ...running,
