...
import { listMacApplications, macApplicationFromPath } from "./macos/processes";
...
export function registerIpc(controller: AppController, window: BrowserWindow): void {
  ...
  ipcMain.handle(IPC.listProcesses, () => listMacApplications());
  ipcMain.handle(IPC.selectExecutable, async () => {
    const result = await dialog.showOpenDialog(window, {
      title: "Выберите приложение macOS",
      buttonLabel: "Добавить",
      properties: ["openFile", "dontAddToRecent"],
      filters: [{ name: "Приложения macOS", extensions: ["app"] }],
    });
    const filePath = result.filePaths[0];
    if (result.canceled || !filePath) return null;
    const selected = await macApplicationFromPath(filePath);
